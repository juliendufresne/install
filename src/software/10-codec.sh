...
{
    if ! dpkg --get-selections| grep -q -E "libavcodec-extra\s*install"
    then
        return 1
    fi
...
function main
{
    ...
    install_package "libavcodec-extra" &>"$output" || {
        error_with_output_file "$output" "Unable to install libavcodec-extra package"
