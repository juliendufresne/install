...
function main
{
    ...
    do
        ...
        generate_ssh_key "$ssh_dir" "$git_provider" || true
    done
...
function generate_ssh_key()
{
    ...
    ssh-keygen -q -t rsa -b 4096 -N "$passphrase" -f "$HOME/.ssh/$git_provider" -C "$(whoami)@$(hostname) for $provider"
