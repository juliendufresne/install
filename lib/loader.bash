...
if [[ -v LOADER_ACTIVE ]] && [[ "$LOADER_ACTIVE" = true ]]
then
...
function include {
	...
	/*|./*|../*)
		...
		;;
	*)
		[[ ${LOADER_FLAGS[$1]+exists} ]] && return

		for __ in "${LOADER_PATHS[@]}"; do
			loader_getcleanpath "$__/$1"

			if [[ ${LOADER_FLAGS[$__]+exists} ]]; then
				LOADER_FLAGS[$1]=.
				return 0
			elif [[ -f $__ ]]; then
...
function loader_addpath {
	for __; do
		...
		if ! [[ ${LOADER_PATHS_FLAGS[$__]+exists} ]]; then
